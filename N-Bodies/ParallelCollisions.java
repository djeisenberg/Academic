...
public class ParallelCollisions {
    ...
    private static long      maxSteps     = 100000L;
    ...
    public static void main(String[] args) throws IOException,
        ...
        ParallelBody bodies[];
        if (args.length <= 1) {
            bodies = new ParallelBody[numBodies];
            for (int i = 0; i < numBodies; i++) {
                int j = i * 6;
                bodies[i] = new ParallelBody(Constants.PARAMETERS[j],
            ...
            bodies = new ParallelBody[numBodies];
            for (int i = 0; i < numBodies; i++) {
                bodies[i] = new ParallelBody(rand(), rand(), rand(),
                                             rand(), mass, radius,
                                             numWorkers);
            }
        }
        ...
        // initialize barrier
        barrier = new Semaphore[Constants.MAX_STAGES][numWorkers];
        ...
        Worker workers[] = new Worker[numWorkers - 1];
        for (int i = 0; i < numWorkers - 1; i++) {
            workers[i] = new Worker(i, bodies);
    ...
    public static void updateForces(ParallelBody bodies[], int id) {
        ...
        for (int i = id; i < bodies.length; i += numWorkers) {
            for (int j = i + 1; j < bodies.length; j++) {
                ...
                int myIndex = id * 8;
                bodies[i].fx[myIndex] += dirx * forceScalar;
                bodies[j].fx[myIndex] -= dirx * forceScalar;
                bodies[i].fy[myIndex] += diry * forceScalar;
                bodies[j].fy[myIndex] -= diry * forceScalar;
            }
        }
        ...
        for (int i = id; i < bodies.length; i += numWorkers) {
            ...
            for (int j = i + 1; j < bodies.length; j++) {
                ...
                if (!Double.isInfinite(increment) &&
                    ...
                    for (int k = 0; k < bodies[i].fx.length; k += 8) {
    ...
    public static void updatePositions(ParallelBody bodies[], int id) {
        double forcex = 0.0;
        double forcey = 0.0;
        for (int i = id; i < bodies.length; i += numWorkers) {
            for (int j = 0; j < bodies[i].fx.length; j += 8) {
    ...
    public static void checkCollisions(ParallelBody bodies[], int id) {
        for (int i = id; i < bodies.length; i += numWorkers) {
            for (int j = i + 1; j < bodies.length; j++) {
    ...
    public static void barrier(int id) {
        ...
        for (int i = 1; i < numWorkers; i *= 2) {
            barrier[stage][id].release();
            barrier[stage][(id + i) % numWorkers].acquireUninterruptibly();
            stage++;
        }
